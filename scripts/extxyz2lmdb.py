...
def main(args: argparse.Namespace) -> None:
    ...
    db_path = os.path.join(args.out_path, "data.lmdb")  # 修改为共享的 LMDB 路径
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--data-path", help="Path to dir containing *.extxyz and *.txt files")
    parser.add_argument("--out-path", help="Directory to save extracted features. Will create if doesn't exist")
    parser.add_argument("--get-edges", action="store_true", help="Store edge indices in LMDB, ~10x storage requirement. Default: compute edge indices on-the-fly.")
    parser.add_argument("--num-workers", type=int, default=1, help="No. of feature-extracting processes or no. of dataset chunks")
