...
"""
这个python文件主要是用来对数据进行处理的，包括数据的读取，数据的预处理，数据的分割等等，另外还有
对模型的一些参数的预处理，调用模型（registry），最终结果的保存位置，checkpoint保存位置等等。
损失函数等相关定义
"""
import datetime
import errno
...
class BaseTrainerDDP(ABC):
    ...
    def save_results(self, predictions, results_file: Optional[str], keys) -> None:
        if results_file is None:
            return
