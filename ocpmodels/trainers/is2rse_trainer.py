...
@registry.register_trainer("is2rve_demo")
class Is2RveTrainerDemo(BaseTrainer):
    """
    ...
    def _compute_loss(self, out, batch_list) -> int:
        ...
        if tag_specific_weights != []:
            ...
            v_mult = self.config["optim"].get("graph_coefficient", 30)
            # 目前使用
...
class Is2RseTrainer(BaseTrainer):
    ...
    def load_task(self) -> None:
        ...
        if self.normalizer.get("normalize_labels", False):
            ...
            if (self.config["model_attributes"].get("regress_forces", False) and
                    "grad_target_mean" in self.normalizer):
    ...
    def _forward(self, batch_list):
        # forward pass.
        out_e, out_v, out_p, out_f = self.model(batch_list)
        out = {
            'energy': out_e,
        ...
        main_graph = self.model.module.main_graph
        batch = batch_list[0]
...
@registry.register_trainer("is2rse1")
class Is2RseTrainer1(BaseTrainer):
    """
    Trainer class for the Structure to Energy & Force (S2EF) and Initial State to
    Relaxed State (IS2RS) tasks.
    ...
    def _compute_loss(self, out, batch_list) -> int:
        ...
        if out['positions']:
            ...
            pos_origin = torch.cat([batch.pos.to(self.device) for batch in batch_list])
            p_mult = self.config["optim"].get("pos_coefficient", 1)
            for pred_pos in out['positions']:
                p_i = 1
                p_i_loss = p_mult * self.loss_fn['vector'](pred_pos,
                                                           pos_origin + (pos_target - pos_origin) * (
                                                                       p_i / len(out['positions'])))
                loss = loss + p_i_loss
                p_i += 1
        # else:
        #     p_loss = None
...
@registry.register_trainer("is2rse2")
class Is2RseTrainer2(BaseTrainer):
    """
    Trainer class for the Structure to Energy & Force (S2EF) and Initial State to
    Relaxed State (IS2RS) tasks.
    ...
    def _forward(self, batch_list):
        # forward pass.
        out_e, out_v, out_p = self.model(batch_list)
        out = {
            'energy': out_e,
            "vector": out_v if out_v is not None else torch.tensor([]),
            "positions": out_p if out_p is not None else torch.tensor([]),
    ...
    def _compute_loss(self, out, batch_list) -> int:
        ...
        if out['vector']:
            ...
            v_origin = torch.cat(
                [batch.v0.to(self.device) for batch in batch_list]
            )  # 更新vector版本
            if self.normalizer.get("normalize_labels", False):
                if "v_mean" in self.normalizer:
                    v_target_normed = self.normalizers["v_target"].norm(v_target)
            ...
            v_mult = self.config["optim"].get("graph_coefficient", 1)
            for pre_vec in out['vector']:
                v_i = 1
                v_i_loss = v_mult * self.loss_fn['vector'](pre_vec,
                                                           v_origin + (v_target - v_origin)*(v_i/len(out['vector'])))
                v_i += 1
                loss = loss + v_i_loss
            # 目前使用
            # v_loss = v_mult * self.loss_fn["vector"](out["vector"],
        ...
        if out['positions']:
            ...
            for pred_pos in out['positions']:
                p_i=1
                p_i_loss = p_mult*self.loss_fn['vector'](pred_pos,
                                                         pos_origin + (pos_target-pos_origin)*(p_i/len(out['positions'])))
                loss = loss + p_i_loss
                p_i+=1
...
@registry.register_trainer("is2rfse")
class Is2RfseTrainer(BaseTrainer):
    """
    Trainer class for the Structure to Energy & Force (S2EF) and Initial State to
    Relaxed State (IS2RS) tasks.
        ...
        if self.config["model_attributes"].get("regress_forces", True):
            ...
            if tag_specific_weights != []:
                ...
                if self.config["optim"].get("loss_force", "l2mae") == "l2mae":
                    ...
                    force_loss = force_mult * weighted_dists_sum
                else:
    ...
    def _compute_metrics(self, out, batch_list, evaluator, metrics={}):
        ...
        if self.normalizer.get("normalize_labels", False):
            ...
            if 'target' in self.normalizers:
                out["energy"] = self.normalizers["target"].denorm(out["energy"])
            if 'grad_target' in self.normalizers:
                out["forces"] = self.normalizers["grad_target"].denorm(
                    out["forces"]
                )
        if not self.config["model_attributes"].get("regress_forces", True):
            target['forces'] = out['forces']
        # print(out, target)
        metrics = evaluator.eval(out, target, prev_metrics=metrics)
        return metrics
