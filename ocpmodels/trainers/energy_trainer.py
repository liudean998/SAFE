...
from ocpmodels.trainers.base_trainer_ddp import BaseTrainerDDP
...
class EnergyTrainer(BaseTrainer):
    ...
    def _forward(self, batch_list):
        ...
        # 假设 output 是一个 tuple，解包
        if isinstance(output, tuple):
            energy_output = output[0]  # 获取第一个元素作为 energy_output
        else:
            energy_output = output

        # 确保 energy_output 是一个张量并进行处理
        if energy_output.shape[-1] == 1:
            energy_output = energy_output.view(-1)
...
@registry.register_trainer("energy_ddp")
class EnergyTrainer(BaseTrainerDDP):
    """
    Trainer class for the Initial Structure to Relaxed Energy (IS2RE) task.
