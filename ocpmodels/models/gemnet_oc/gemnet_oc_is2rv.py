...
class GemNetOCRV(GemNetOC):
    ...
    def __init__(
        ...
        # self.atom_emb_tags = AtomEmbeddingTags(emb_size_atom, num_elements, tags_size=32)
...
@registry.register_model("gemnet_is2rd")
class GemNetOCRD(GemNetOC):
    """
    Arguments
    ---------
    num_atoms (int): Unused argument
    ...
    def __init__(
        ...
        self.out_mlp_D = torch.nn.Sequential(*out_mlp_D)
        self.out_d = Dense(
            emb_size_edge, 1, bias=False, activation=None
        )
        out_initializer = get_initializer(output_init)
        self.out_d.reset_parameters(out_initializer)
...
class GemNetOCRVE(GemNetOC):
    ...
    def get_bases_v(
        ...
        vector_dis = torch.cat((main_graph['vector'], main_graph['distance'].unsqueeze(1)), dim=1)
        vector_dis = vector_dis.view(-1)
        a2a_vector_dis = torch.cat((a2a_graph['vector'], a2a_graph['distance'].unsqueeze(1)), dim=1)
        a2a_vector_dis = a2a_vector_dis.view(-1)
        basis_rad_main_raw_v = self.radial_basis(vector_dis)
        # print(size)
        basis_rad_main_raw = self.reshape_vector(basis_rad_main_raw_v)
        # basis_rad_main_raw = basis_rad_main_raw_v.clone()
        # -------
        # Calculate triplet angles
        cosφ_cab = inner_product_clamped(
        ...
        if self.edge_atom_interaction:
            ...
            basis_rad_cir_e2a_raw = self.reshape_vector(basis_rad_cir_e2a_raw)
        if self.atom_interaction:
            basis_rad_a2a_raw = self.radial_basis_aint(a2a_vector_dis)  # 原是a2a_graph distance, 由于这里使a2a_graph=main_graph
            basis_rad_a2a_raw = self.reshape_vector(basis_rad_a2a_raw)
        # Shared Down Projections
        bases_qint = {}
        if self.quad_interaction:
...
class GemNetOCRV2E(GemNetOC):
    def __init__(
        ...
        self.num_targets = num_targets
        assert num_blocks > 0
        self.num_blocks = int(num_blocks / 2)
        self.num_blocks_e = int(num_blocks / 2)
        self.extensive = extensive
        ...
        self.init_shared_basis_layers_e(
            num_radial_e, num_spherical, emb_size_rbf, emb_size_cbf, emb_size_sbf
        )

        # Embedding blocks
        self.atom_emb = AtomEmbedding(emb_size_atom, 83)
        self.atom_emb_e = AtomEmbedding(emb_size_atom_e, 83)
    ...
    def init_basis_functions_e(
        ...
        self.radial_basis_e = RadialBasis(
            num_radial=num_radial,
            cutoff=self.cutoff,
            rbf=rbf,
            envelope=envelope,
        ...
        radial_basis_spherical_e = RadialBasis(
            num_radial=num_radial,
            cutoff=self.cutoff,
            rbf=rbf_spherical,
            envelope=envelope,
        ...
        if self.atom_edge_interaction:
            self.radial_basis_aeaint_e = RadialBasis(
                num_radial=num_radial,
                cutoff=self.cutoff_aeaint,
                rbf=rbf,
                envelope=envelope,
            ...
            self.cbf_basis_aeint_e = CircularBasisLayer(
                num_spherical,
                radial_basis=radial_basis_spherical_e,
                cbf=cbf,
                scale_basis=scale_basis,
            )
        if self.edge_atom_interaction:
            ...
            radial_basis_spherical_aeaint_e = RadialBasis(
                num_radial=num_radial,
                cutoff=self.cutoff_aeaint,
                rbf=rbf_spherical,
                envelope=envelope,
            ...
            self.cbf_basis_eaint_e = CircularBasisLayer(
                num_spherical,
                radial_basis=radial_basis_spherical_aeaint_e,
                cbf=cbf,
                scale_basis=scale_basis,
            )
        if self.atom_interaction:
            self.radial_basis_aint_e = RadialBasis(
                num_radial=num_radial,
                cutoff=self.cutoff_aint,
                rbf=rbf,
                envelope=envelope,
        ...
        self.cbf_basis_tint_e = CircularBasisLayer(
            num_spherical,
            radial_basis=radial_basis_spherical_e,
            cbf=cbf,
            scale_basis=scale_basis,)

    def init_shared_basis_layers_e(
            self,
            num_radial,
            num_spherical,
            emb_size_rbf,
        ...
        if self.quad_interaction:
            self.mlp_rbf_qint_e = Dense(
                num_radial,
                emb_size_rbf,
                activation=None,
                bias=False,
            )
            self.mlp_cbf_qint_e = BasisEmbedding(
                num_radial, emb_size_cbf, num_spherical
            )
            self.mlp_sbf_qint_e = BasisEmbedding(
                num_radial, emb_size_sbf, num_spherical ** 2
            )

        if self.atom_edge_interaction:
            self.mlp_rbf_aeint_e = Dense(
                num_radial,
                emb_size_rbf,
                activation=None,
                bias=False,
            )
            self.mlp_cbf_aeint_e = BasisEmbedding(
                num_radial, emb_size_cbf, num_spherical
            )
        if self.edge_atom_interaction:
            self.mlp_rbf_eaint_e = Dense(
                num_radial,
                emb_size_rbf,
                activation=None,
                bias=False,
            )
            self.mlp_cbf_eaint_e = BasisEmbedding(
                num_radial, emb_size_cbf, num_spherical
            )
        if self.atom_interaction:
            self.mlp_rbf_aint_e = BasisEmbedding(num_radial, emb_size_rbf)

        self.mlp_rbf_tint_e = Dense(
            num_radial,
            emb_size_rbf,
            activation=None,
            bias=False,
        )
        self.mlp_cbf_tint_e = BasisEmbedding(
            num_radial, emb_size_cbf, num_spherical
        )

        # Share the dense Layer of the atom embedding block accross the interaction blocks
        self.mlp_rbf_h_e = Dense(
            num_radial,
            emb_size_rbf,
            activation=None,
            bias=False,
        )
        self.mlp_rbf_out_e = Dense(
            num_radial,
            emb_size_rbf,
            activation=None,
            bias=False,
    ...
    def get_bases(
        ...
        """Calculate and transform basis functions."""
        basis_rad_main_raw = self.radial_basis_e(main_graph["distance"])
        # Calculate triplet angles
        cosφ_cab = inner_product_clamped(
            main_graph["vector"][trip_idx_e2e["out"]],
            main_graph["vector"][trip_idx_e2e["in"]],
        )
        basis_rad_cir_e2e_raw, basis_cir_e2e_raw = self.cbf_basis_tint_e(
            main_graph["distance"], cosφ_cab
        )

        if self.atom_edge_interaction:
            basis_rad_a2ee2a_raw = self.radial_basis_aeaint_e(
            ...
            basis_rad_cir_a2e_raw, basis_cir_a2e_raw = self.cbf_basis_aeint_e(
        ...
        if self.edge_atom_interaction:
            ...
            basis_rad_cir_e2a_raw, basis_cir_e2a_raw = self.cbf_basis_eaint_e(
                a2ee2a_graph["distance"], cosφ_cab_e2a
            )
        if self.atom_interaction:
            basis_rad_a2a_raw = self.radial_basis_aint_e(a2a_graph["distance"])
    ...
    def forward(self, data):
        ...
        (
            ...
            trip_idx_a2e_e,
            trip_idx_e2a_e,
        ) = self.subselect_graph_from_main_graph(data, main_graph) # 以main_graph，取其他图
