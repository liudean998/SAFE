...
from .layers.atom_update_block import OutputBlockMask, OutputBlockStru, OutputBlockStru1, OutputBlockStruForce, OutputBlockStruForce1
...
from .layers.radial_basis import RadialBasis,  RadialBasisVec
from .layers.spherical_basis import CircularBasisLayer, SphericalBasisLayer, CircularBasisLayerVec, SphericalBasisLayerVec
from .utils import (
    get_inner_idx,
...
@registry.register_model("gemnet_is2rve_demo")
...
class GemNetOCRSE(GemNetOC):
    ...
    def forward(self, data):
        ...
        # (nEdges, emb_size_edge)
        x_E, x_V, x_P, x_F = self.out_blocks[0](h, m, basis_output, idx_t)
        # (nEdges, emb_size_edge)
        xs_E = [x_E]
        xs_V = [x_V]
        xs_P = [x_P]
        ...
        for i in range(self.num_blocks):
            ...
            x_E, x_V, x_P, x_F = self.out_blocks[i + 1](h, m, basis_output, idx_t)
            xs_E.append(x_E)
            xs_V.append(x_V)
            xs_P.append(x_P)
            xs_F.append(x_F)
        ...
        return E_t, V_t, P_t, F_t

@registry.register_model("gemnet_is2rse1")
# 自定义force输出部分
class GemNetOCRSE1(GemNetOC):
    def __init__(
            self,
            num_atoms: Optional[int],
            bond_feat_dim: int,
    ...
    def forward(self, data):
        ...
        return E_t, V_t, P_t, F_t
@registry.register_model("gemnet_is2rse2")
# 测试共享层
class GemNetOCRSE2(GemNetOC):
    def __init__(
            self,
            num_atoms: Optional[int],
            bond_feat_dim: int,
...
@registry.register_model("gemnet_is2rse3")
# 共享层+原始力输出
class GemNetOCRSE3(GemNetOC):
    def __init__(
            self,
            num_atoms: Optional[int],
            bond_feat_dim: int,
    ...
    def forward(self, data):
        ...
        if self.regress_forces:
            if self.direct_forces:
                x_F = self.out_mlp_F(torch.cat(xs_V, dim=-1))
                F_st = self.out_forces(x_F.float())
                if self.forces_coupled:  # enforce F_st = F_ts
                    nEdges = idx_t.shape[0]
                    id_undir = repeat_blocks(
...
@registry.register_model("gemnet_is2rfse")
class GemNetOCRFSE(GemNetOC):
    def __init__(
            self,
            num_atoms: Optional[int],
            bond_feat_dim: int,
    ...
    def forward(self, data):
        ...
        # (nEdges, emb_size_edge)
        x_E, x_V, x_P, x_F = self.out_blocks[0](h, m, basis_output, idx_t,
                                           out_vector=self.update_v,
                                           out_pos=self.update_p)
        # (nEdges, emb_size_edge)
        xs_V = [x_V]
        xs_E = [x_E]
        xs_P = [x_P]
