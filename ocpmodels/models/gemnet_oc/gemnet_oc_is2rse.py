...
from .layers.atom_update_block import OutputBlockMask, OutputBlockStru, OutputBlockStru1
...
from .layers.radial_basis import RadialBasis,  RadialBasisVec, RadialBasisVecDirct
from .layers.spherical_basis import (CircularBasisLayer, SphericalBasisLayer,
                                     CircularBasisLayerVec, SphericalBasisLayerVec)
from .utils import (
    get_inner_idx,
    inner_product_clamped,
    repeat_blocks,
)
@registry.register_model("gemnet_is2rse")
class GemNetOCRSE(GemNetOC):
    def __init__(
            self,
    ...
    def forward(self, data):
        ...
        if self.regress_forces and self.direct_forces:
            x_F = self.out_mlp_F(torch.cat(xs_E, dim=-1))
            F_t = self.out_forces(x_F)
            F_t = F_t.squeeze(1)
        else:
            F_t = None
        return E_t, V_t, P_t, F_t, main_graph


@registry.register_model("gemnet_is2rse_tags")
class GemNetOCRSETags(GemNetOC):
    def __init__(
            self,
            num_atoms: Optional[int],
            bond_feat_dim: int,
        ...
        # Embedding blocks
        self.atom_emb = AtomEmbeddingTags(emb_size_atom, num_elements)
        self.edge_emb = EdgeEmbedding(
            emb_size_atom, num_radial, emb_size_edge, activation=activation
        )
    ...
    def forward(self, data):
        ...
        # Embedding block
        h = self.atom_emb(atomic_numbers, tags)
        # (nAtoms, emb_size_atom)
        m = self.edge_emb(h, basis_rad_raw, main_graph["edge_index"])
...
@registry.register_model("gemnet_is2rse_tags_dict")
class GemNetOCRSETagsDict(GemNetOC):
    def __init__(
            self,
            num_atoms: Optional[int],
            bond_feat_dim: int,
        ...
        if self.update_v:
            ...
            self.out_mlp_V = torch.nn.Sequential(*out_mlp_V)
            self.out_v = Dense(
                emb_size_edge, 3, bias=False, activation='relu'
            )
        if self.update_p:
            out_mlp_P = [
                            Dense(
    ...
    def init_basis_functions(
        ...
        # ------------使用单位化的Vetor和distance做rbf嵌入
        self.radial_basis = RadialBasisVecDirct(
            num_radial=int(num_radial / 4),
            cutoff=self.cutoff,
            rbf=rbf,
            envelope=envelope,
        ...
        radial_basis_spherical = RadialBasisVecDirct(
            num_radial=int(num_radial / 4),
            cutoff=self.cutoff,
            rbf=rbf_spherical,
            envelope=envelope,
        ...
        if self.quad_interaction:
            radial_basis_spherical_qint = RadialBasisVecDirct(
                num_radial=int(num_radial / 4),
                cutoff=self.cutoff_qint,
                rbf=rbf_spherical,
                envelope=envelope,
            ...
            self.cbf_basis_qint = CircularBasisLayerVecDirct(
                num_spherical,
                radial_basis=radial_basis_spherical_qint,
                cbf=cbf,
                scale_basis=scale_basis,
        ...
        if self.atom_edge_interaction:
            self.radial_basis_aeaint = RadialBasisVecDirct(
                num_radial=int(num_radial / 4),
                cutoff=self.cutoff_aeaint,
                rbf=rbf,
                envelope=envelope,
            ...
            self.cbf_basis_aeint = CircularBasisLayerVecDirct(
                num_spherical,
                radial_basis=radial_basis_spherical,
                cbf=cbf,
                scale_basis=scale_basis,
        ...
        if self.edge_atom_interaction:
            ...
            radial_basis_spherical_aeaint = RadialBasisVecDirct(
                num_radial=int(num_radial / 4),
                cutoff=self.cutoff_aeaint,
                rbf=rbf_spherical,
                envelope=envelope,
            ...
            self.cbf_basis_eaint = CircularBasisLayerVecDirct(
                num_spherical,
                radial_basis=radial_basis_spherical_aeaint,
                cbf=cbf,
                scale_basis=scale_basis,
        ...
        if self.atom_interaction:
            self.radial_basis_aint = RadialBasisVecDirct(
                num_radial=int(num_radial / 4),
                cutoff=self.cutoff_aint,
                rbf=rbf,
                envelope=envelope,
        ...
        self.cbf_basis_tint = CircularBasisLayerVecDirct(
            num_spherical,
            radial_basis=radial_basis_spherical,
            cbf=cbf,
            scale_basis=scale_basis,
    ...
    def get_graphs_and_indices_lmdb(self, data):
        ...
        if (
            ...
            (
                ...
                data,
                cutoff=self.cutoff_aint,
                max_neighbors=self.max_neighbors_aint,
            ...
            if self.otf_graph or self.max_neighbors == 50:
                select_neighbors = None
            else:
                select_neighbors = self.max_neighbors
            a2a_graph = self.subselect_edges(
                data=data,
                graph=a2a_graph,
