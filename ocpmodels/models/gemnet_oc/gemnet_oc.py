...
class GemNetOCT(BaseModel):
    ...
    def get_graphs_and_indices(self, data):
        ...
        edge_dist = torch.norm(data.v1, dim=1)
        edge_vector = data.v1 / edge_dist[:, None]
        # edge_vector = -distance_vec / edge_dist[:, None]
        ...
        if (
            ...
            # )
            # a2a_graph_other = {
            #     'edge_index': a2a_graph['edge_index'][:, data.a2a_edge_mask],
            #     'distance': a2a_graph['distance'][data.a2a_edge_mask],
            #     'cell_offset': a2a_graph['cell_offset'][data.a2a_edge_mask],
            #     'vector': a2a_graph['vector'][data.a2a_edge_mask],
            #     'num_neighbors': a2a_graph['num_neighbors']
            # }
            # if a2a_graph_other['edge_index'] is not None:
            #     a2a_graph = {
            #         'edge_index': torch.cat((main_graph['edge_index'], a2a_graph_other['edge_index']),dim=1),
            #         'distance': torch.cat((main_graph['distance'], a2a_graph_other['distance']),dim=0),
            #         'cell_offset': torch.cat((main_graph['cell_offset'], a2a_graph_other['cell_offset']),dim=0),
            #         'vector': torch.cat((main_graph['vector'], a2a_graph_other['vector']),dim=0),
            #         'num_neighbors': a2a_graph['num_neighbors'],
            #     }
            #     a2a_graph, _ = self.symmetrize_edges(
            #         a2a_graph, data.batch
            #     )
            # else:
            #     a2a_graph = main_graph
            # sorted_indices = torch.argsort(a2a_graph['edge_index'][1])
            # a2a_graph['edge_index'] = a2a_graph['edge_index'][:, sorted_indices]
            # a2a_graph['distance'] = a2a_graph['distance'][sorted_indices]
            # a2a_graph['cell_offset'] = a2a_graph['cell_offset'][sorted_indices]
            # main_graph = self.subselect_graph(
            #     data,
            #     a2a_graph,
            #     self.cutoff,
