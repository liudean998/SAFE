...
from .radial_basis import GaussianBasis, RadialBasis,  RadialBasisVec, RadialBasisVecDirct
...
class CircularBasisLayerVec(torch.nn.Module):
    """
    2D Fourier Bessel Basis
    ...
    def forward(self, D_ca,V_ca, cosφ_cab):
        rad_basis = self.radial_basis(D_ca, V_ca)  # (num_edges, num_radial)
        cir_basis = self.cosφ_basis(cosφ_cab)  # (num_triplets, num_spherical)
...
class CircularBasisLayerVecDirct(torch.nn.Module):
    """
    2D Fourier Bessel Basis
    ...
    def __init__(
        self,
        num_spherical: int,
        radial_basis: RadialBasisVecDirct,
        cbf: dict,
        scale_basis: bool = False,
    ) -> None:
        super().__init__()
...
class SphericalBasisLayer(torch.nn.Module):
    ...
    def __init__(
        self,
        num_spherical: int,
        radial_basis: RadialBasis or RadialBasisVec,
        sbf: dict,
        scale_basis: bool = False,
    ) -> None:
        super().__init__()
...
class SphericalBasisLayerVec(torch.nn.Module):
    """
    3D Fourier Bessel Basis
    ...
    def forward(self, D_ca, V_ca, cosφ_cab, θ_cabd):
        rad_basis = self.radial_basis(D_ca, V_ca)
        sph_basis = self.spherical_basis(cosφ_cab, θ_cabd)
        # (num_quadruplets, num_spherical**2)
