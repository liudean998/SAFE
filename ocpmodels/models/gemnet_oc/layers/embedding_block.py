...
# 增加原子标签
    def __init__(self, emb_size: int, num_elements: int, tags_size=128) -> None:
        ...
        self.embeddings = torch.nn.Embedding(num_elements, emb_size-tags_size)
        # init by uniform distribution
        torch.nn.init.uniform_(
            self.embeddings.weight, a=-np.sqrt(3), b=np.sqrt(3)
        )
        if tags_size  > 0:
            self.embeddings_tags = torch.nn.Embedding(5, tags_size)
            # init by uniform distribution
            torch.nn.init.uniform_(
...
class EdgeEmbeddingVectorDis(torch.nn.Module):
    """
    Edge embedding based on the concatenation of atom embeddings
    and a subsequent dense layer.
