...
class SO2EquivariantGraphAttentionERD(torch.nn.Module):
    """
    SO2EquivariantGraphAttention: Perform MLP attention + non-linear message passing
        SO(2) Convolution with radial function -> S2 Activation -> SO(2) Convolution -> attention weights and non-linear messages
        attention weights * non-linear messages -> Linear
    ...
    def __init__(
        ...
        super(SO2EquivariantGraphAttentionERD, self).__init__()
    ...
    def forward(
        ...
        edge_message = x_message.clone()
        # print(edge_message.embedding.shape)

        # Compute the sum of the incoming neighboring messages for each target node
        x_message._reduce_edge(edge_index[1], len(x.embedding))
        # print(x_message.embedding.shape)
        # print(edge_message.embedding.shape)

        # Project
        out_embedding = self.proj(x_message)

        edge_out = self.edge_liner(edge_message)

        # print(out_embedding.embedding.shape)
        # print(edge_out.embedding.shape)
        # print(x_message.embedding.shape)
        # import time
        # time.sleep(10000)
        return out_embedding,  edge_out, x_message