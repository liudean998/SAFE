...
from .transformer_block import (
    ...
    TransBlockV2,
    SO2EquivariantGraphAttentionERD
)
...
class EquiformerV2_OC20(BaseModel):
    ...
    def forward(self, data):
        ...
        # print(x.embedding.shape, 'ori')
        # print(edge_degree.embedding.shape, 'edge')
        x.embedding = x.embedding + edge_degree.embedding
        # print(x.embedding.shape, 'add')
        # import time
        # time.sleep(1000)
...
@registry.register_model("equiformer_v2_ERD")
class EquiformerV2_OC20(BaseModel):
    """
    Equiformer with graph attention built upon SO(2) convolution and feedforward network built upon S2 activation
    ...
    def __init__(
        ...
        if self.regress_forces:
            self.force_block = SO2EquivariantGraphAttentionERD(
                self.sphere_channels,
                self.attn_hidden_channels,
                self.num_heads,
                self.attn_alpha_channels,
    ...
    def forward(self, data):
        ...
        # Final layer norm
        x.embedding = self.norm(x.embedding)
        # print(x.embedding.shape, 'ooo')

        ###############################################################
        # Energy estimation
        ###############################################################
        node_energy = self.energy_block(x)
        # print(node_energy.embedding.shape)
        # import time
        # time.sleep(1000)
        pos_offset = node_energy.embedding.narrow(1, 1, 3)
        pos_offset = pos_offset.view(-1, 3)
        edge_out = edge_distance_vec + pos_offset[edge_index[0]] - pos_offset[edge_index[1]]
        ...
        ###############################################################
        # if self.regress_forces:
        #     pos_offset, edge_out, x = self.force_block(
        #         x, atomic_numbers, edge_distance, edge_index
        #     )
        #     pos_offset = pos_offset.embedding.narrow(1, 1, 3)
        #     pos_offset = pos_offset.view(-1, 3)
        #
        #     edge_out = edge_out.embedding.narrow(1, 0, 1)
        #     edge_out = edge_out.view(-1)



        return energy,  edge_out, pos_offset, None, self.main_graph
