...
model_state_dict['out_blocks.0.scale_rbf_V'] = torch.nn.Parameter(torch.tensor(8.6099, dtype=torch.float32))
model_state_dict['out_blocks.1.scale_rbf_V'] = torch.nn.Parameter(torch.tensor(8.0398, dtype=torch.float32))
model_state_dict['out_blocks.2.scale_rbf_V'] = torch.nn.Parameter(torch.tensor(8.3941, dtype=torch.float32))
model_state_dict['out_blocks.3.scale_rbf_V'] = torch.nn.Parameter(torch.tensor(8.7591, dtype=torch.float32))
model_state_dict['out_blocks.4.scale_rbf_V'] = torch.nn.Parameter(torch.tensor(8.8912, dtype=torch.float32))
...
# 步骤 3: 保存更新后的状态字典
new_checkpoint_path = '/home/liud/Documents/code/ocp/configs/s2ef/all/gemnet/scaling_factors/gemnet-oc-is2rv.pt'  # 替换为新的 .pt 文件路径
